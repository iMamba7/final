...
class data_manager:
    ...
    def get_company_data(self, selection: str, company: str):
        ...
        if selection == 'paisa':
            cursor.execute("SELECT date, name, box, dozen, total_items, price, total_price, flag FROM data WHERE id = ?", (company,))
        elif selection == 'weight':
            cursor.execute("SELECT date, name, box, dozen, total_items, weight, total_weight, flag FROM data WHERE id = ?", (company,))
